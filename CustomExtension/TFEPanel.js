...
const dllInputPath =
  __dirname + "/../TestProject/TestProject/bin/Debug/net5.0/TestProject.dll";
const pdbInputPath =
  __dirname + "/../TestProject/TestProject/bin/Debug/net5.0/TestProject.pdb";
...
var TFEPanel = /** @class */ (function () {
  ...
  TFEPanel.kill = function () {
    var _a;
    (_a = TFEPanel.currentPanel) === null || _a === void 0
      ? void 0
      : _a.dispose();
              ...
              switch (data.command) {
                case "executeTestMethod": {
                  if (
                    getServers()
                      .filter((x) => x.isActive)
                      .filter((y) => y.sites.length != 0).length === 0
                  ) {
                    vscode.window.showInformationMessage(
                      "Please configure atleast one site"
                    );
                  } else {
                    //startTimer();
                    vscode.window.showInformationMessage(
...
(function subscribeResumeTopic() {
  getServers()
    .filter((x) => x.isActive)
    .forEach((server) => {
      server.subscription.resumeSubscription =
        server.service.pubsubService.SubscribeResumeTopic({
          ClientName: "TFE",
        });
...
async function resumeExecution() {
  ...
  getServers()
    .filter((x) => x.isActive)
    .forEach((server) => {
      server.service.testMethodService.ResumeExecution({}, (err) => {
...
async function stopExecution() {
  ...
  getServers()
    .filter((x) => x.isActive)
    .forEach((server) => {
      server.service.testMethodService.StopExecution({}, (err) => {
      ...
      server.service.testMethodService.UpdateDLL(
        ...
        (err) => {
          ...
          if (err) {
            ...
            vscode.window.showInformationMessage(
              "Rebuild Test Project Successfully..."
            );
          }
        }
      );
...
function executeTestMethodInServer() {
  getServers()
    ...
    .forEach((server) => {
      server.service.testMethodService.ExecuteTestMethod(
        getTFEData(),
        (err) => {
