              ...
              switch (data.command) {
                ...
                case "syncDatalogConfigData":
                  selfWebView.postMessage({
                    command: "updateDatalogConfig",
                    datalogConfig: getDatalogConfig(),
                  });
                  break;
              }
              return [2 /*return*/];
...
function getNewDataAsString() {
  var newData = getDatalogData().splice(0, 1000000);
  var newDataAsString = "";
  ...
  try {
    ...
    lineReader.eachLine(logFilePath, (data, last) => {
      if (last) {
        ...
        selfWebView.postMessage({
          command: "updateDatalogData",
          datalogData: datalogData.reverse(),
        });
        setTimeout(refreshDatalogData, getDatalogConfig().refreshRate);
